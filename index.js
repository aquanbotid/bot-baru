...
async function starts() {
	...
	fs.existsSync('./AQUANID.json') && client.loadAuthInfo('./AQUANID.json')
	client.on('connecting', () => {
		start('2', 'Menyambungkan Hubungan Yang Sudah terputus :)')
	})
	client.on('open', () => {
		success('2', 'Sukses, Sc AQUAN BOT ID')
	})
	await client.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./AQUANID.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "WELCOME BERSAMA GUA BOT AQUAN ID MEmEK", "conversation");
      }
...
if (anu.action == 'add' && !mem.includes(client.user.jid)) {
			 ...
			 const bosco2 = bosco1.message["ephemeralMessage"] ? bosco1.message.ephemeralMessage : bosco1
                ...
                teks = `🌿 *KONNICHIWA* @${num.split('@')[0]}\n🌿 *Bɪᴏ* : *${thu.status}*\n🌿 *Mᴇᴍʙᴇʀs : ${memeg}*\n🌿 *Wᴇʟᴄᴏᴍᴇ Tᴏ* \n *${mdata.subject}*\n*Subscribe My Channel*\n*https://youtube.com/channel/UCK_-10nUvPSFbeA1n1hLm_A*\n🌿 *Dᴏɴᴛ Fᴏʀɢᴇᴛ Dɪsᴄʀɪᴘᴛɪᴏɴ* \n *${mdata.desc}*\n ${time_wel} -  ${time_welc}`
                welcomeBut = [{buttonId:`#owner`,buttonText:{displayText:'OWNER'},type:1}, {buttonId:`bebantot`,buttonText:{displayText:'LINK GROUP BOT AQUAN'},type:1}]
...
let settingstatus = 0;
    if (new Date() * 1 - settingstatus > 1000) {
      ...
      await client.setStatus(`Aktif selama ${uptime} | spam/vc/telfon?block | Sewa Bot Only |By Lord AQUAN🗿`).catch((_) => _);
...
*Instagram : @ff.phasa*
*YouTube : DKZ OW*
*Link Yt : https://youtube.com/channel/UCK_-10nUvPSFbeA1n1hLm_A*
*WhatsApp : wa.me/6283163205227*
...
break
case 'bebantot':
                reply(`*https://chat.whatsapp.com/ImEIW7cobVt3QkoUjHM5fx*`)
                break
case 'rules':
...
_${prefix}narutobanner_
`
sendButMessage(from, menu, '_*BY AQUAN BOT ID*_', [
{buttonId: `${prefix}owner`, buttonText: {displayText: 'OWNER BOT:)'}, type: 1},
{buttonId: `${prefix}donasi`, buttonText: {displayText: 'DONASI BOT ;)'}, type: 1}
]);
break
...
_*MENU TELAH DIHILANGKAN OWNER*_
